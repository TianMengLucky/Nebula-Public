...
public class YandereLover() : DefinedAllocatableModifierTemplate("YandereLover", "YL", Yandere.Team.Color),DefinedAllocatableModifier
{
    ...
    void HasAssignmentRoutine.TryAssign(IRoleTable roleTable){
        if (roleTable is RoleTable table)
        {
            ...
            foreach (var r in table.roles)
            {
                ...
                has.Add(r.playerId);
                var players = table.roles.Where(n => has.Contains(n.playerId)).ToArray();
